...
CHUNKED_OUTPUT_DIR = "/home/nachman/sctn-env/lib/python3.11/site-packages/python_sctn/Project_Geo/project/MyCode/chunked_output_norm"

# Processing configuration
LOAD_FROM_CHUNKED = False  # True: Load pre-computed features, False: Full pipeline
PROCESS_HUMAN_FILE = True  # True: Process 40Sen_30Sec_stomping_30sec_quiet.csv through full pipeline
...
if PROCESS_HUMAN_FILE:
    ...
    print(f"🎯 Full pipeline from raw signals to trained models")
...
class OptimizedSCTN:
    ...
    def train_multiclass_snn(self, training_features, training_labels, epochs=150, learning_rate=0.15):
        """Train the SCTN model for multi-class classification using 3 separate SCTN neurons."""
        if self.signal_optimization == 'human':
            self.feature_normalizer = MinMaxScaler(feature_range=(0.05, 0.95))
        else:
            self.feature_normalizer = MinMaxScaler(feature_range=(0.1, 0.9))
        ...
        # ===== CREATE 3 SEPARATE SCTN NEURONS (one for each class) =====
        unique_classes = np.unique(training_labels)
        self.num_classes = len(unique_classes)
        
        # Initialize 3 separate SCTN neurons instead of unused single neuron
        self.class_neurons = []
        for class_idx in range(self.num_classes):
            neuron = create_SCTN()
            
            if class_idx == 0:  # Human class
                neuron.synapses_weights = np.random.normal(0, 0.05, self.input_size).astype(np.float64)
                neuron.threshold_pulse = 8.0
            elif class_idx == 1:  # Car class
                neuron.synapses_weights = np.random.normal(0, 0.04, self.input_size).astype(np.float64)
                neuron.threshold_pulse = 12.0
            else:  # Background class
                neuron.synapses_weights = np.random.normal(0, 0.035, self.input_size).astype(np.float64)
                neuron.threshold_pulse = 14.0
            
            neuron.activation_function = BINARY
            neuron.theta = 0.0
            neuron.reset_to = 0.0
            neuron.membrane_should_reset = True
            neuron.label = f"MultiClass_SCTN_Class_{class_idx}"
            
            self.class_neurons.append(neuron)
        
        if self.num_classes >= 3:
            print(f"   🎯 Enhanced training: Using {self.num_classes} separate SCTN neurons")
        
        # ===== TRAINING WITH ACTUAL SCTN DYNAMICS =====
        for epoch in range(epochs):
            if epoch <= 50:
                current_lr = learning_rate * 1.5
            elif epoch <= 120:
            ...
            for sample_idx in training_indices:
                ...
                # Train each SCTN neuron with proper spiking dynamics
                for class_idx in range(self.num_classes):
                    target = 1.0 if class_idx == true_class else 0.0
                    
                    # Use actual SCTN forward propagation
                    neuron = self.class_neurons[class_idx]
                    neuron.membrane_potential = 0.0
                    neuron.index = 0
                    
                    synaptic_activation = np.dot(features, neuron.synapses_weights)
                    neuron.membrane_potential = synaptic_activation
                    spike_output = neuron._activation_function_binary()
                    
                    # Calculate error and update with SCTN dynamics
                    class_error = target - spike_output
                    
                    # Update actual SCTN neuron weights and threshold
                    weight_update = current_lr * class_error * features
                    neuron.synapses_weights += weight_update
                    
                    threshold_update = current_lr * class_error * 0.025
                    neuron.threshold_pulse += threshold_update
    ...
    def predict_multiclass_snn(self, test_features):
        """Generate multi-class predictions using the 3 trained SCTN neurons."""
        if not self.training_complete:
            raise RuntimeError("SCTN model must be trained before making predictions")
        
        if not hasattr(self, 'class_neurons') or self.class_neurons is None:
        ...
        for feature_vector in normalized_features:
            ...
            # Get output from each SCTN neuron
            for class_idx in range(self.num_classes):
                neuron = self.class_neurons[class_idx]
                neuron.membrane_potential = 0.0
                neuron.index = 0
                ...
                # Use actual SCTN forward propagation
                synaptic_activation = np.dot(feature_vector, neuron.synapses_weights)
                neuron.membrane_potential = synaptic_activation
                spike_output = neuron._activation_function_binary()
                
                class_activations.append(synaptic_activation)
                class_spike_outputs.append(spike_output)
            
            # Winner-takes-all decision based on membrane potentials
            class_activations = np.array(class_activations)
            predicted_class = np.argmax(class_activations)
            
            # Convert to probabilities using softmax on membrane potentials
...
def process_file_in_chunks(file_path, chunk_duration=120, num_processes=15, min_chunk_size=10):
    ...
    output_dir = os.path.join(os.path.dirname(os.path.abspath(__file__)), "chunked_output_norm", file_stem)
    os.makedirs(output_dir, exist_ok=True)
...
def process_human_file_pipeline():
    """
    Process the 40Sen_30Sec_stomping_30sec_quiet.csv file through the full pipeline.
    Each sensor (column) is processed as a separate chunk for both human and background sections.
    """
    print("\n🧠 MULTI-SENSOR GEOPHONE PROCESSOR (EXACT PIPELINE)")
    print("=" * 80)
    ...
    # Output directories
    CHUNKED_OUTPUT_HUMAN = "/home/nachman/sctn-env/lib/python3.11/site-packages/python_sctn/Project_Geo/project/MyCode/chunked_output_norm/human_new"
    CHUNKED_OUTPUT_BG = "/home/nachman/sctn-env/lib/python3.11/site-packages/python_sctn/Project_Geo/project/MyCode/chunked_output_norm/human_nothing_new"
    ...
    def process_and_save(signal, section_name, chunk_idx, output_dir, clk_resonators, duration, num_processes=8, time_arr=None):
        """Process and save a single chunk using the exact pipeline."""
    ...
    if PROCESS_HUMAN_FILE:
        ...
        try:
            ...
            if success:
                ...
                print("   - /chunked_output_norm/human_new/")
                print("   - /chunked_output_norm/human_nothing_new/")
            else:
        ...
        try:
            ...
            if LOAD_FROM_CHUNKED:
                ...
                print("      1. Verify chunked_output_norm directory exists with processed data")
...
# ========================================================================
# MULTI-OUTPUT SCTN NEURON (Alternative Architecture)
# ========================================================================

class MultiOutputSCTN:
    """
    Single SCTN neuron with multiple output channels for multi-class classification.
    
    This implements a true multi-output spiking neuron where:
    - Single input processing layer
    - Multiple output channels (one per class)
    - Shared membrane dynamics with class-specific outputs
    """
    
    def __init__(self, input_size, num_classes=3, signal_optimization=None):
        self.input_size = input_size
        self.num_classes = num_classes
        self.signal_optimization = signal_optimization
        self.feature_normalizer = None
        ...
        # Single SCTN neuron for input processing
        self.base_neuron = None
        
        # Multiple output channels
        self.output_weights = None
        self.output_thresholds = None
        self.output_biases = None
        
    def _initialize_multi_output_neuron(self):
        """Initialize single SCTN with multiple output channels."""
        # Base SCTN neuron for input processing
        neuron = create_SCTN()
        
        if self.signal_optimization == 'human':
            neuron.synapses_weights = np.random.normal(0, 0.025, self.input_size).astype(np.float64)
            neuron.threshold_pulse = 10.0
        elif self.signal_optimization == 'car':
            neuron.synapses_weights = np.random.normal(0, 0.04, self.input_size).astype(np.float64)
            neuron.threshold_pulse = 15.0
        else:
            neuron.synapses_weights = np.random.normal(0, 0.03, self.input_size).astype(np.float64)
            neuron.threshold_pulse = 12.0
        ...
        neuron.reset_to = 0.0
        neuron.membrane_should_reset = True
        neuron.label = "MultiOutput_SCTN"
        
        # Initialize output channels (class-specific processing)
        self.output_weights = np.random.normal(0, 0.02, (self.num_classes, self.input_size)).astype(np.float64)
        self.output_thresholds = np.array([8.0, 12.0, 14.0][:self.num_classes], dtype=np.float64)
        self.output_biases = np.zeros(self.num_classes, dtype=np.float64)
        
        return neuron
    
    def _multi_output_forward(self, feature_vector):
        """Forward propagation through multi-output SCTN."""
        # Step 1: Base SCTN processing
        self.base_neuron.membrane_potential = 0.0
        self.base_neuron.index = 0
        
        base_activation = np.dot(feature_vector, self.base_neuron.synapses_weights)
        self.base_neuron.membrane_potential = base_activation
        base_spike = self.base_neuron._activation_function_binary()
        
        # Step 2: Multi-output channel processing
        class_outputs = []
        class_activations = []
        
        for class_idx in range(self.num_classes):
            # Each output channel processes the input independently but influenced by base
            channel_activation = np.dot(feature_vector, self.output_weights[class_idx])
            
            # Combine base neuron influence with channel-specific processing
            combined_activation = (base_activation * 0.3) + (channel_activation * 0.7) + self.output_biases[class_idx]
            
            # Class-specific thresholding
            class_spike = 1.0 if combined_activation > self.output_thresholds[class_idx] else 0.0
            
            class_outputs.append(class_spike)
            class_activations.append(combined_activation)
        
        return class_outputs, class_activations, base_spike
    
    def train_multi_output_snn(self, training_features, training_labels, epochs=150, learning_rate=0.15):
        """Train the multi-output SCTN neuron."""
        if self.signal_optimization == 'human':
            self.feature_normalizer = MinMaxScaler(feature_range=(0.05, 0.95))
        else:
            self.feature_normalizer = MinMaxScaler(feature_range=(0.1, 0.9))
        ...
        normalized_features = self.feature_normalizer.fit_transform(training_features)
        self.base_neuron = self._initialize_multi_output_neuron()
        
        unique_classes = np.unique(training_labels)
        self.num_classes = len(unique_classes)
        
        print(f"   🎯 Training single multi-output SCTN with {self.num_classes} output channels")
        ...
        for epoch in range(epochs):
            ...
            for sample_idx in training_indices:
                ...
                # Forward pass through multi-output neuron
                class_outputs, class_activations, base_spike = self._multi_output_forward(features)
                
                # Update base neuron (shared processing)
                target_base = 1.0  # Always want base neuron to be active for any input
                base_error = target_base - base_spike
                base_weight_update = current_lr * base_error * features * 0.1  # Smaller updates for stability
                self.base_neuron.synapses_weights += base_weight_update
                
                # Update output channels (class-specific)
                for class_idx in range(self.num_classes):
                    target = 1.0 if class_idx == true_class else 0.0
                    class_error = target - class_outputs[class_idx]
                    
                    # Update output channel weights
                    output_weight_update = current_lr * class_error * features
                    self.output_weights[class_idx] += output_weight_update
                    
                    # Update output thresholds
                    threshold_update = current_lr * class_error * 0.025
                    self.output_thresholds[class_idx] += threshold_update
                    
                    # Update output biases
                    bias_update = current_lr * class_error * 0.1
                    self.output_biases[class_idx] += bias_update
        
        self.training_complete = True
        return True
    
    def predict_multi_output_snn(self, test_features):
        """Generate predictions using the multi-output SCTN."""
        if not self.training_complete:
            raise RuntimeError("Multi-output SCTN must be trained before making predictions")
        ...
        for feature_vector in normalized_features:
            class_outputs, class_activations, base_spike = self._multi_output_forward(feature_vector)
