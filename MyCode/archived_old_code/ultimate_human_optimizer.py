...
Test EVERY TRAIN split (50%-98% TRAIN) with EVERY epoch (20-2000) to find absolute optimal combination
"""
...
class UltimateHumanSCTNClassifier:
    """Ultimate optimized SCTN classifier for finding the best configuration"""
...
def test_split_epoch_combination(X, y, test_size, epochs, random_state=42):
    """Test a specific split and epoch combination"""
    try:
        # Split data
        ...
        classifier = UltimateHumanSCTNClassifier(input_size=X.shape[1])
...
def save_progress(results, filename="ultimate_optimization_progress.json"):
    """Save progress to file"""
    with open(filename, 'w') as f:
        json.dump(results, f, indent=2)

def load_progress(filename="ultimate_optimization_progress.json"):
...
def run_ultimate_optimization():
    """Run the ultimate optimization: every split with every epoch"""
    print("🚀 ULTIMATE HUMAN CLASSIFIER OPTIMIZATION")
    print("=" * 60)
    print("Testing EVERY TRAIN split (50%-98% TRAIN) with EVERY epoch (20-2000)")
    print("This will find the ABSOLUTE OPTIMAL configuration!")
    print()
    ...
    # Define test parameters - USER WANTS 50-98% TRAIN, which means 2-50% TEST
    train_sizes = np.arange(0.50, 0.99, 0.01)  # 50% to 98% TRAIN in 1% steps (49 splits)
    test_sizes = 1.0 - train_sizes             # Corresponding test sizes (50% to 2%)
    epochs_range = np.arange(20, 2001, 10)     # 20 to 2000 in steps of 10 (198 epochs)
    
    total_experiments = len(test_sizes) * len(epochs_range)
    
    print(f"\n🎯 ULTIMATE OPTIMIZATION PARAMETERS:")
    print(f"   Train sizes: {len(train_sizes)} splits (50% to 98% TRAIN)")
    print(f"   Test sizes: Corresponding (50% to 2% TEST)")
    ...
    # Track best results for each split
    best_per_split = {}
    ...
    for i, (train_size, test_size) in enumerate(zip(train_sizes, test_sizes)):
        ...
        for j, epochs in enumerate(epochs_range):
            # Skip if already completed (check by train_pct)
            ...
            # Progress indicator (every 100 experiments)
            ...
            accuracy, status = test_split_epoch_combination(X, y, test_size, epochs)
            ...
            # Save progress every 100 experiments
            if experiment_count % 100 == 0:
    ...
    print(f"\n⏱️  Ultimate optimization completed in {elapsed_time/60:.1f} minutes")
    ...
    # Final results
    print(f"\n🏆 ULTIMATE OPTIMAL CONFIGURATION FOUND:")
    print("=" * 70)
    ...
    print(f"🎯 ULTIMATE WINNER: {best_config['train_pct']:.0f}%/{best_config['test_pct']:.0f}% split with {best_config['epochs']} epochs")
    ...
    print(f"\n📈 TOP 10 ULTIMATE CONFIGURATIONS:")
    print("=" * 85)
    ...
    # Best epochs analysis
    print(f"\n📊 BEST EPOCHS BY SPLIT:")
    print("-" * 50)
    for train_size in sorted(best_per_split.keys())[:10]:  # Show first 10
    ...
    results_file = "ultimate_human_optimization_complete.csv"
    final_results_df.to_csv(results_file, index=False)
    
    # Save final progress
    progress['completed'] = True
    progress['completion_time'] = datetime.now().isoformat()
    save_progress(progress, "ultimate_optimization_final.json")
...
if __name__ == "__main__":
    print("🚀 STARTING ULTIMATE HUMAN OPTIMIZATION")
    print("Testing 50%-98% TRAIN sizes with 20-2000 epochs!")
    print("This will take a while - finding the ABSOLUTE BEST configuration!")
    print("=" * 70)
    
    optimization_results = run_ultimate_optimization()
    
    if optimization_results:
        best = optimization_results['best_config']
        print(f"\n✅ ULTIMATE OPTIMIZATION COMPLETE!")
        print(f"🏆 ABSOLUTE WINNER: {best['train_pct']:.0f}%/{best['test_pct']:.0f}% split with {best['epochs']} epochs")
        print(f"🎯 ULTIMATE ACCURACY: {best['accuracy']:.4f} ({best['accuracy']*100:.2f}%)")
        print(f"🚀 This is the ABSOLUTE OPTIMAL configuration for human footstep detection!")
    else:
        print(f"\n❌ Ultimate optimization failed!") 